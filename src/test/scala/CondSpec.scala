...
class CondSpec extends AnyFlatSpec with Matchers {
  ...
  "Equals" should "check equals" in {
    parser ? "(cond (== 5 5) 1 (== 9 (* 3 3)) 0)".state flatMap { expr =>
      expr.asInstanceOf[Expression].eval(env)
    } should be(Success(1))

    parser ? "(cond (== 5.01 5) 1 (== 9 (* 3 3)) 0)".state flatMap { expr =>
      expr.asInstanceOf[Expression].eval(env)
    } should be(Success(0))

    parser ? "(cond (== 5 5.0000001) 1 (* 3 3))".state flatMap { expr =>
      expr.asInstanceOf[Expression].eval(env)
    } should be(Success(9))
  }

  "Greater" should "true if x > y " in {
    parser ? "(cond (> 5.0001 5) (* 2 3.14) 0)".state flatMap { expr =>
      expr.asInstanceOf[Expression].eval(env)
    } should be(Success(6.28))

    parser ? "(cond (> 5 5) 1 (> 3.14 3) 3.14)".state flatMap { expr =>
      expr.asInstanceOf[Expression].eval(env)
    } should be(Success(3.14))

    parser ? "(cond (> 5 5.1) 1 (> 3.14 3.14) 3.14 (* 2 3.14))".state flatMap { expr =>
      expr.asInstanceOf[Expression].eval(env)
    } should be(Success(6.28))
  }
